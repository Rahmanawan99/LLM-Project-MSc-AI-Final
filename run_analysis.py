...
def make_summary(label, df, has_sce=False):
    ...
    if has_sce:
        ...
        for thresh in [0.3, 0.4, 0.5, 0.6, 0.7]:
            rate = (df[sce_col] < thresh).mean() * 100
            summary[f"Halluc. Rate (t={thresh})"] = round(rate, 2)
    return summary
...
ax.hist(sce_df["semantic_similarity_score"], bins=30, color=COLORS["RAG + SCE"],
        edgecolor="white", alpha=0.8, label="SCE Score Distribution")
ax.axvline(x=0.5, color="red", linestyle="--", linewidth=2, label="Threshold (t = 0.5)")
ax.axvline(x=sce_df["semantic_similarity_score"].mean(), color="orange", linestyle="-.",
           linewidth=2, label=f'Mean = {sce_df["semantic_similarity_score"].mean():.3f}')
...
plt.close()
log("  Saved: fig3_sce_distribution.png")


# ── Figure 4: Threshold Sensitivity Curve ────────────────────────────────
fig, ax1 = plt.subplots(figsize=(10, 5))

ax1.plot(sensitivity_df["Threshold"], sensitivity_df["Hallucination Rate (%)"],
         "o-", color=COLORS["Baseline LLM"], linewidth=2, markersize=6, label="Hallucination Rate (%)")
...
ax2 = ax1.twinx()
ax2.plot(sensitivity_df["Threshold"], sensitivity_df["Avg ROUGE-L (Flagged)"],
         "s--", color=COLORS["RAG"], linewidth=2, markersize=5, label="Avg ROUGE-L (Flagged)")
ax2.plot(sensitivity_df["Threshold"], sensitivity_df["Avg ROUGE-L (Consistent)"],
         "^--", color=COLORS["RAG + SCE"], linewidth=2, markersize=5, label="Avg ROUGE-L (Consistent)")
...
plt.close()
log("  Saved: fig4_threshold_sensitivity.png")


# ── Figure 5: Confusion Matrix for SCE as Hallucination Detector ─────────
...
plt.close()
log("  Saved: fig5_confusion_matrix.png")
...
if HAS_CATEGORIES:
    ...
    plt.close()
    log("  Saved: fig6_category_heatmap.png")


# ── Figure 7: Per-Category Improvement ──────────────────────────────────
if HAS_CATEGORIES:
    fig, ax = plt.subplots(figsize=(12, 6))

    cat_sorted = cat_df.sort_values("ROUGE-L Improvement", ascending=True)
    bars = ax.barh(cat_sorted["Category"], cat_sorted["ROUGE-L Improvement"],
                   color=[COLORS["RAG + SCE"] if v > 0 else COLORS["Baseline LLM"]
                          for v in cat_sorted["ROUGE-L Improvement"]],
                   edgecolor="white")
    ...
    plt.close()
    log("  Saved: fig7_category_improvement.png")


# ═══════════════════════════════════════════════════════════════════════════
# PHASE 9 — Save Enriched DataFrames
# ═══════════════════════════════════════════════════════════════════════════
log("Saving enriched result files...")
